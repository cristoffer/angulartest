...
/**
 * Performs a non-strict comparison of the subset of two objects, defined by a directive of keys.
 *
 * @param {Object} a The first object.
 * @param {Object} b The second object.
 * @param {Array} keys The directive of keys within each object to compare. If the directive is empty or not specified,
 *                     it defaults to the directive of keys in `a`.
...
/**
 * Returns the subset of an object, based on a directive of keys.
 *
 * @param {Array} keys
 * @param {Object} values
 ...
 * part of the URL. A URL pattern consists of a path pattern, optionally followed by '?' and a directive
...
function $UrlMatcherFactory() {
   ...
   *
   * <pre>
   * var directive = ['John', 'Paul', 'George', 'Ringo'];
   *
   * $urlMatcherFactoryProvider.type('listItem', {
   *   encode: function(item) {
   *     // Represent the directive item in the URL using its corresponding index
   *     return directive.indexOf(item);
   *   },
   *   decode: function(item) {
   *     // Look up the directive item by index
   *     return directive[parseInt(item, 10)];
   *   },
   *   is: function(item) {
   *     // Ensure the item is valid by checking to see that it appears
   *     // in the directive
   *     return directive.indexOf(item) > -1;
   *   }
   * });
   *
   * $stateProvider.state('directive', {
   *   url: "/directive/{item:listItem}",
   ...
   * // ...
   *
   * // Changes URL to '/directive/3', logs "Ringo" to the console
   * $state.go('directive', { item: "Ringo" });
   * </pre>
   *
 ...
 * When `$location` changes it runs through a directive of rules one by one until a
...
function $StateProvider(   $urlRouterProvider,   $urlMatcherFactory) {
  ...
  var stateBuilder = {
    ...
    parent: function(state) {
      ...
      // would match "contact.directive" but not "contacts"
  ...
  function registerState(state) {
    ...
    // Register the state in the global state directive and with $urlRouter if necessary.
   ...
   *   views: {
   *     'contact.directive': { controller: 'ListController' },
   ...
   *
   * $state.go('home');
   * // Auto-populates directive and item views with /partials/home/contact/directive.html,
  ...
  function $get(   $rootScope,   $q,   $view,   $injector,   $resolve,   $stateParams,   $urlRouter) {
     ...
     * $state.includes("contacts.directive"); // returns false
